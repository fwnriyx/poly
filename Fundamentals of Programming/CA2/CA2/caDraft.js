...
class memberGroup {
    ...
    largestNumber() {
        //Creates empty array to store names
        let largestName = [];
        //Initialises first number of points earned to be the largest
        let largest = members.tMembers[0].PointsEarned;
        // Checks if there is anyone with a higher amount of points and slides it into largestName
        for (var i = 0; i < members.memberListLength(); i++) {
            if (largest < members.tMembers[i].PointsEarned) {
                members.tMembers[i].PointsEarned = largest;
                largestName.push(members.tMembers[i].Name);
            }
            var highestPoints = largestName[largestName.length - 1];
            var highestState = "Highest member: " + highestPoints;
        }
        return highestState;
    }
    smallestNumber() {
        ...
        var smallest = members.tMembers[0].PointsEarned;

        for (var i = 0; i < members.memberListLength(); i++) {
            if (smallest > members.tMembers[i].PointsEarned) {
                members.tMembers[i].PointsEarned = smallest;
                smallestName.push(members.tMembers[i].Name);
            }
            var lowestPoints = smallestName[smallestName.length - 1];
            var lowestState = "Lowest member: " + lowestPoints;
        }
        return lowestState;
    }
    addPoints(amSpent) {
    ...
    changeRank() {
        for (var i = 0; i < this.tMembers.length; i++) {
            ...
            else if (members.tMembers[i].MembershipClass >= 5000) {
                members.tMembers[i].MembershipClass = "Platinum"
            }
            else if (members.tMembers[i].MembershipClass >= 500) {
